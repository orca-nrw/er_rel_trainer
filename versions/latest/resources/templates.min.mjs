import{html,render}from"https://eild-nrw.github.io/er_rel_trainer/versions/latest/libs/lit/lit.min.js";export{render};export function main(app,show_solution){const data=app.getValue();const notation=app.notations[data.notation];const phrase_nr=data.results.length;const phrase=data.phrases[phrase_nr-1];const result=data.results[phrase_nr-1];const is_binary=phrase.entities.length===2;const is_recursive=is_binary&&phrase.entities[0]===phrase.entities[1]&&!notation.mirrored;const is_single=!phrase.solution.find((value=>value!=="1"&&value!=="c"));const is_multi=!phrase.solution.find((value=>value!=="cn"&&value!=="n"));const is_many=is_binary&&!is_single&&!is_multi;const is_hierarchy=!phrase.relation;const arrows_mask=15872;const fk_mask=496;const heading=result.correct===undefined?"main_heading":"feedback_"+(result.correct?"correct":"failed");return html`
        ...
        <section>
          <div class="d-flex align-items-center">
            ...
            <select id="notation-input" class="form-select ms-2" @change=${event=>app.events.onNotation(event.target.value)}>
              ${Object.values(app.notations).sort(((a,b)=>a.title.localeCompare(b.title))).map((({key:key,title:title})=>html`<option value="${key}" ?disabled=${!is_binary&&app.notations[key].swap} ?selected=${data.notation===key}>${title}</option>`))}
            </select>
          </div>
        </section>
    ...
    <main class="border rounded-bottom border-top-0 px-4 py-2 text-nowrap">
      <div>
        ...
        <section ?data-hidden=${!(app.comments&&app.comments.wrong&&result.correct===false)}>
          ...
          ${comment("null_fk",is_many&&(phrase.solution.includes("c")||phrase.solution.toString()==="c,c")&&(()=>{const left=phrase.solution[0]==="c";const table=result.input[left?1:2];const attr=table&&table[left?2:1];const fk=left?64:32;return attr&fk&&attr&2})())}

          <!-- Wrong Alternate Key -->
          ${comment("ak",is_hierarchy&&result.input.find(((table,i)=>i>1&&table&&table[1]&1<<5&&!(table[1]&1<<3)))||is_single&&(()=>{const c1=phrase.solution.toString()==="c,1";const table=result.input[c1?2:1];const attr=table&&table[c1?1:2];const fk=c1?32:64;return attr&fk&&!(attr&8)})())}
        ...
        <section ?data-hidden=${!(app.comments&&app.comments.correct&&result.correct===true)}>
          ...
          ${comment("mandatory",(()=>{switch(phrase.solution.toString()){case"1,1":case"1,n":case"n,1":case"c,n":case"n,c":return true}return false})())}
  ...
  `;function diagram(){return html`
    ...
    `;function connection(entity){return entity?html`
      ...
      `:html`<div></div>`}function entity(nr){return phrase.entities[nr-1]?html`
        <div class="entity p-3">
          ${phrase.entities[nr-1]}
        </div>
      `:html`<div></div>`}function line(nr){if(!nr)return html`<div></div>`;if(nr===8)return html`
        <div class="line8">
      ...
      `;return html`<div class="line${nr}"></div>`}}function scheme(is_solution){const input=is_solution?result.solution:result.input;const solution=!is_solution&&(result.alternate_solution||result.solution);return html`
      <form id="scheme" @submit=${event=>{event.preventDefault();!is_solution&&app.events.onSubmit()}}>
        <section class="tables px-2 py-4">
          ${is_recursive&&recursive()||is_binary&&binary()||is_hierarchy&&(phrase.entities.length===3?hierarchy_3():hierarchy_4())||(phrase.entities.length===3?multi_3():multi_4())}
        </section>
      </form>
    `;function binary(){return html`
        <div class="binary">
          ${table(1)}
          ${arrow("l",2,1)}
          ${connection("h",1,2)}
      ...
      `}function recursive(){return html`
        <div class="recursive">
          ${connection("rd",1,2)}
          ${connection("h",1,2)}
          ${connection("dl",1,2)}
      ...
      `}function hierarchy_3(){return html`
        <div class="multi-3">
          ${connection("rd",1,2)}
          ${connection("h",1,2)}
          ${arrow("r",2,1)}
      ...
      `}function hierarchy_4(){return html`
        <div class="multi-4">
          <div></div>
          <div></div>
          <div></div>
      ...
      `}function table(table){const input_table=input[table]&&input[table].map((attr=>attr&~arrows_mask));const solution_table=solution&&solution[table]&&solution[table].map((attr=>attr&~arrows_mask));if(!input_table)return html`
          <div>
      ...
      `;function attr(i){const input_attr=input_table[i];const solution_attr=solution&&solution_table&&solution_table[i];return html`
        ...
        `}function key(id){let bit,color;switch(id){case"pk":bit=4;color="primary";break;case"ak":bit=8;color="success";break}return html`
        ...
        `}}function connection(line_id,a,b,c,d){if(line_id==="x"){const v=isConnected(a,b);const h=isConnected(c,d);if(!v&&!h)return html`<div></div>`;if(v&&!h)return line("v");if(!v&&h)return line("h");if(v&&h)return line("x")}return isConnected(a,b)?line(line_id):html`<div></div>`}function line(line_id){if(line_id==="x")return html`
          <div class="line-x">
        ...
        `;return html`<div class="line-${line_id}"></div>`}function arrow(arrow_id,from,to){if(isConnected(from,to)){const $arrow=()=>{switch(arrow_id){case"l":case"r":return html`
                ...
                `}};const $line=()=>line(arrow_id==="r"||arrow_id==="l"?"h":"v");if(app.auto_arrows)return hasRef(from,to)?$arrow():$line();const input_attr=input[from][to];const input_bit=input_attr&1<<9+to;const solution_attr=solution&&solution[from]&&solution[from][to];const solution_bit=solution_attr&1<<9+to;return html`
        ...
        `}return html`<div class="arrow-${arrow_id}"></div>`}}function isConnected(a,b){return hasRef(a,b)||hasRef(b,a)}function hasRef(from,to){const value=result.input[from]&&(result.input[to]||is_recursive&&to===2)&&result.input[from][to];return!!(value&1<<4||value&1<<5||value&1<<6||value&1<<7||value&1<<8)}function comment(index,condition){return condition?html`<div class="alert alert-info my-2 text-wrap" role="alert" data-lang="comment_${index}">${app.text["comment_"+index]}</div>`:html`<div></div>`}}export function legend(app){const ids=["1","c","n","cn"];return html`
  ...
  `}export function tableDialogTitle(app,table){const data=app.getValue();const phrase=data.phrases[data.results.length-1];return html`
    <span data-lang='main_table'>${app.text.main_table}</span>: ${table&&phrase.entities[table-1]||phrase.relation||app.text.hierarchy_is}
  `}export function tableDialogBody(app,table){const data=app.getValue();const phrase=data.phrases[data.results.length-1];const result=data.results[data.results.length-1];return html`
    <table class="table table-bordered table-striped text-nowrap">
      <tbody>
        ${result.input[table].map(((_,i,arr)=>attr(i)))}
      </tbody>
    </table>
  `;function attr(attr){return html`
      <tr>
        <td>
    ...
    `}}export function tableDialogFooter(app){return html`
